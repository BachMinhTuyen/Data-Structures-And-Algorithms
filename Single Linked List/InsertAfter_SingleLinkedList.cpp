...
void insertAfter(SList& list, Node* q, Node* p);
...
int main()
{
	...
	printf("Chen sau phan tu co gia tri: ");
	scanf_s("%d", &x);
...
void insertAfter(SList& list, Node* q, Node* p)
{
	//nếu không tồn tại Node p hoặc Node q thì không thực hiện
	if (q == NULL || p == NULL)
		return;
	//q->next trỏ đến đâu thì p->next cũng trỏ tới đó
	p->next = q->next;
	//q->next trỏ vào Node p
	q->next = p;
	//Nếu Node q là vị trí cuối danh sách thì con trỏ tail sẽ trỏ sang Node p
	if (q == list.tail)
		list.tail = p;
}
void insertLast(SList& list, Node* p)
